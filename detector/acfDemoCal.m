...
dataDir='D:\work\code\computer_civision\datasets\caltech\';
skip=3;
...
opts.posGtDir=[dataDir 'usa-train' '/annotations'];
opts.posImgDir=[dataDir 'usa-train' '/images'];
...
imgNms=bbGt('getFiles',{[dataDir 'test']});
I=imread(imgNms{1}); tic, bbs=acfDetect(I,detector); toc
figure(1); im(I); bbApply('draw',bbs); pause(.1);

%% test detector and plot roc (see acfTest)
[~,~,gt,dt]=acfTest('name',opts.name,'imgDir',[dataDir 'usa-test/images/set06/V001'],...
  'gtDir',[dataDir 'usa-test/annotations/set06/V001'],'pLoad',[pLoad, 'hRng',[50 inf],...
